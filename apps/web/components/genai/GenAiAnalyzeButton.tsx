...
export function GenAiAnalyzeButton() {
  ...
  async function handleClick() {
    setLoading(true)
    try {
      const res = await fetch('/api/genai/analyze', { method: 'POST' })
  ...
  return (
    ...
    >
      {loading ? <><Loader2 size={12} className="animate-spin" />分析中...</> : <>今すぐ分析<ArrowRight size={12} /></>}
    </button>
  )
}
